(() => {
      ...
      for (let i = 0; i < modifiers.length; i++) {
        ...
        switch (mod) {
          case "params":
            ...
            if (!isObject(scope2[modifiers[i + 1]])) break;
  ...
  function AlpinePlugin(Alpine2) {
    ...
    Alpine2.directive("render", (el, { modifiers, expression }, { evaluate: evaluate2, cleanup }) => {
      const click = (e) => {
    ...
    Alpine2.directive("template", (el, { modifiers, expression }, { effect: effect3, cleanup }) => {
    ...
    Alpine2.directive("file-drop", (el, { expression }, { effect: effect3, cleanup }) => {
      var target;
      const evaluate2 = Alpine2.evaluateLater(el, expression);
      effect3(() => evaluate2((value) => {
    ...
    Alpine2.directive("draggable", (el, { expression }, { effect: effect3, cleanup }) => {
      var target;
      const evaluate2 = Alpine2.evaluateLater(el, expression);
      effect3(() => evaluate2((value) => {
        if (!value || value === target) return;
  ...
  // esbuild-app:./{hello,dropdown,todo,dashboard}.js
  var hello_dropdown_todo_dashboard_exports = {};
  __export(hello_dropdown_todo_dashboard_exports, {
    chart: () => chart,
    dashboard: () => dashboard,
    dropdown: () => dropdown,
    hello: () => hello,
  ...
  function initInterceptors(data22, cleanup = () => {
  }) {
    ...
    let recurse = (obj, basePath = "") => {
      Object.entries(Object.getOwnPropertyDescriptors(obj)).forEach(([key, { value, enumerable }]) => {
        ...
        if (typeof value === "object" && value !== null && value._x_interceptor) {
          obj[key] = value.initialize(data22, path, key, cleanup);
        } else {
  ...
  function interceptor(callback, mutateObj = () => {
    ...
    let obj = {
      initialValue: void 0,
      _x_interceptor: true,
      initialize(data22, path, key, cleanup) {
        return callback(this.initialValue, () => get(data22, path), (value) => set(data22, path, value), path, key, cleanup);
      }
    };
    mutateObj(obj);
    return (initialValue) => {
      if (typeof initialValue === "object" && initialValue !== null && initialValue._x_interceptor) {
        ...
        obj.initialize = (data22, path, key, cleanup) => {
          let innerValue = initialValue.initialize(data22, path, key, cleanup);
          obj.initialValue = innerValue;
          return initialize(data22, path, key, cleanup);
        };
      } else {
...
${expression ? 'Expression: "' + expression + '"\n\n' : ""}`, el);
  ...
  directive("model", (el, { modifiers, expression }, { effect: effect3, cleanup }) => {
    ...
    if (isCloning) {
      ...
      if (hasChangeModifier) {
        listeners.push(on2(el, "change", modifiers, syncValue));
      }
      if (hasBlurModifier) {
        listeners.push(on2(el, "blur", modifiers, syncValue));
        if (el.form) {
          let form = el.form;
      ...
      if (hasEnterModifier) {
        listeners.push(on2(el, "keydown", modifiers, (e) => {
          if (e.key === "Enter")
            syncValue(e);
        }));
      }
      ...
      removeListener = on2(el, event, modifiers, (e) => {
  ...
  directive("data", (el, { expression }, { cleanup }) => {
    ...
    if (dataToReconcile?.reactiveData) {
      ...
      reconcileData(reactiveData, data22);
  ...
  // index.js
  var app2 = window.app = app;
  Object.assign(app2, app_exports);
  Object.assign(app2.components, hello_dropdown_todo_dashboard_exports);
  Object.assign(app2.templates, hello_dashboard_dropdown_example_todo_exports);
  app2.debug = 1;
  app2.start();
  $on(document, "alpine:init", () => {
    AlpinePlugin(module_default);
  });
